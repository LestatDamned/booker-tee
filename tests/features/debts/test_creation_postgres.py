...
@pytest.mark.asyncio
async def test_postgres_deletes_unused_debt_and_its_opening_transfer() -> None:
    ...
    try:
        ...
        await delete_context(sessions, context)
        await engine.dispose()


@pytest.mark.asyncio
async def test_postgres_give_loan_is_balanced_atomic_and_idempotent() -> None:
    assert TEST_DATABASE_URL is not None
    engine = create_async_engine(TEST_DATABASE_URL, pool_pre_ping=True)
    sessions = async_sessionmaker(engine, expire_on_commit=False)
    context, funding_account = await seed_context(sessions)
...
@pytest.mark.asyncio
async def test_postgres_creation_failure_rolls_back_the_debt_account() -> None:
    assert TEST_DATABASE_URL is not None
    engine = create_async_engine(TEST_DATABASE_URL, pool_pre_ping=True)
    sessions = async_sessionmaker(engine, expire_on_commit=False)
...
@pytest.mark.asyncio
async def test_postgres_payment_retry_and_undo_keep_ledger_consistent() -> None:
    assert TEST_DATABASE_URL is not None
    engine = create_async_engine(TEST_DATABASE_URL, pool_pre_ping=True)
    sessions = async_sessionmaker(engine, expire_on_commit=False)
...
@pytest.mark.asyncio
async def test_postgres_payment_failure_rolls_back_principal_operation() -> None:
    assert TEST_DATABASE_URL is not None
    engine = create_async_engine(TEST_DATABASE_URL, pool_pre_ping=True)
    sessions = async_sessionmaker(engine, expire_on_commit=False)
    context, settlement_account = await seed_context(sessions)
...
async def delete_context(
    ...
    async with sessions() as session:
        await session.execute(delete(Workspace).where(Workspace.id == context.workspace.id))
        await session.execute(delete(User).where(User.id == context.user.id))
        await session.commit()
