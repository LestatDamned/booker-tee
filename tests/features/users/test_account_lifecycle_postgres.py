...
async def test_email_change_is_confirmed_once_rotates_session_and_rechecks_collision() -> None:
    ...
    try:
        async with sessions() as session:
            ...
            token = parse_qs(urlparse(requested.messages[1].text.splitlines()[2]).fragment)[
                "token"
            ][0]
        ...
        async with sessions() as session:
            ...
            collision_token = parse_qs(
                urlparse(requested.messages[1].text.splitlines()[2]).fragment
            )["token"][0]
