...
async def test_deactivate_is_atomic_and_restore_does_not_resurrect_runtime() -> None:
    ...
    try:
        ...
        await cleanup(sessions, user_id, (target_id, fallback_id))
        await engine.dispose()


async def cleanup(sessions: async_sessionmaker[Any], user_id, workspace_ids) -> None:
    async with sessions() as session:
