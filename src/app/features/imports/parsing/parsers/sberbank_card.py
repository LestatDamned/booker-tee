...
def parse_sberbank_row(row: SberbankOperationRow) -> SberbankParsedRow:
    match = ROW_START_RE.match(row.row_start)
    if match is None:
        return SberbankParsedRow(
...
def build_sberbank_draft(
    parsed_row: SberbankParsedRow,
    *,
    row_index: int,
    context: SberbankParserContext,
) -> RawTransactionDraft:
    if not is_recognized_sberbank_row(parsed_row):
    ...
    return build_raw_transaction_draft(
        row_index=row_index,
        raw_payload={
            **raw_payload(parsed_row.raw_row, context.statement_period),
            "source_row_id": source_row_id,
            "operation_time": parsed_row.operation_time,
            "category_code": parsed_row.category_code,
            "auth_code": parsed_row.auth_code,
        },
        ...
        currency_raw=None,
        balance_after_raw=parsed_row.balance_after_raw,
        ...
        normalized_confidence=Decimal("0.9200"),
        normalization_errors=normalization_errors,
    )


def is_recognized_sberbank_row(parsed_row: SberbankParsedRow) -> bool:
    return parsed_row.operation_date_raw is not None and parsed_row.amount_raw is not None
...
def build_unrecognized_sberbank_draft(
    parsed_row: SberbankParsedRow,
    *,
    row_index: int,
    context: SberbankParserContext,
    ...
    return build_raw_transaction_draft(
        row_index=row_index,
        raw_payload=raw_payload(parsed_row.raw_row, context.statement_period),
        operation_date_raw=None,
        posting_date_raw=None,
        description_raw=parsed_row.description_raw,
        amount_raw=None,
        ...
        operation_date=None,
        posting_date=None,
        description_normalized=parsed_row.description_raw,
        amount=None,
        ...
        normalization_errors=("Sberbank row format was not recognized.",),
    )


def signed_sberbank_amount(raw: str | None, normalization_errors: list[str]) -> Decimal | None:
    try:
