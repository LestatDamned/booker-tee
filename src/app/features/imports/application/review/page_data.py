...
class ImportReviewPageDataLoader:
    ...
    async def _load_references(self, context: WorkspaceContext) -> ImportReviewReferences:
        return ImportReviewReferences(
            accounts=await AccountService(self.session).list_active_accounts(context.workspace.id),
    ...
    async def _load_transfer_data(
        ...
        *,
        ...
        transfer_suggestions = TransferSuggestionUseCase(self.session)
        raw_transactions = getattr(document, "raw_transactions", [])
        return ImportReviewTransferData(
            suggestions=await transfer_suggestions.list_for_document(
                workspace_id=workspace_id,
                raw_transactions=raw_transactions,
            ),
            existing_suggestions=(
                await transfer_suggestions.list_existing_manual_for_document(
                    workspace_id=workspace_id,
                    raw_transactions=raw_transactions,
                )
            ),
