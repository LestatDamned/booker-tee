...
[`REACT_FRONTEND_DESIGN.md`](REACT_FRONTEND_DESIGN.md). Для подробных
исторических discoveries повторяющихся строк можно использовать
...
используется только после `REACT_FRONTEND_DESIGN.md` и только как источник
...
- Properties lifecycle actions now return the properties page with inline
  feedback for expected archive/restore errors.
...
- Categories now use the same recent-created feedback pattern through
  `recent_category_id`, compact feedback above the list and calm row highlight.
...
- for users/workspaces, pass `recent_entity_id` into the
