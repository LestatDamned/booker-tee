...
`WorkbenchRow` сохраняет один семантический контракт во всех feature:

| Слот          | Содержание                                                     |
| ------------- | -------------------------------------------------------------- |
| `description` | Основной факт: что произошло                                   |
| `details`     | Контекст записи: счёт, источник или контрагент                 |
| `meta`        | Классификация: тип, категория, объект и значимый lifecycle     |
| `signals`     | Проблемы, предупреждения, дубликаты и причины блокировки       |
| `value`       | Денежный результат                                             |
| `aside`       | Следующее доступное действие через `ActionStack`               |
| `expansion`   | Реальный inline-workflow: редактирование, проверка или источник |

Feature не повторяет в `details` контекст, уже однозначно заданный страницей:
например, строка на странице счёта не дублирует название этого счёта. Одинаковая
семантика слотов важнее механически одинакового набора полей.
...
| `WorkbenchPanel`           | Создание сущности или редактирование page-level identity справа, пока контекст остаётся видимым |
| `ExpansionPanel`           | Исправление записи коллекции непосредственно под её рабочей строкой                             |
...
Page-level identity на detail     -> WorkbenchPanel справа
Запись внутри рабочей коллекции   -> ExpansionPanel под её строкой
...
edit collection record    -> ExpansionPanel под выбранной row
edit page-level identity  -> WorkbenchPanel справа
```
